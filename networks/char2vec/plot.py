...
def plot_from_files(save_dir, logfiles, savefilename):
  ...
  for df in dic.values():
    ...
    for elm in df:
      ...
      elif elm == "time_per_epoch":   ax2.plot(df["epoch"],df[elm],color=(1,0,0,0.2))
      elif " to " in elm: ax3.plot(df["epoch"],df[elm])
      else: ax.plot(df["epoch"], df[elm])
  ...
  ax3.legend(h3, l3, loc='lower right')
